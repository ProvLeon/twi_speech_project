...
import wandb
from audiomentations import Compose, AddGaussianNoise, TimeStretch, PitchShift # Import augmentations
...
def preprocess_function(examples, feature_extractor, max_duration_s=5, augmentations=None):
    """
    Preprocesses audio data for the Wav2Vec2 model, with optional augmentation.
    """
    audio_arrays = [x["array"] for x in examples["local_path"]]
    sampling_rate = 16000 # We've already resampled to 16kHz

    # Apply augmentations if provided (only for training)
    if augmentations:
        augmented_audio = []
        for audio in audio_arrays:
            augmented_audio.append(augmentations(samples=audio, sample_rate=sampling_rate))
...
def run_hf_training(metadata_csv: str):
    ...
    train_dataset = dataset["train"].map(
        lambda x: preprocess_function(x, feature_extractor, augmentations=augmentations),
        remove_columns=[col for col in dataset["train"].column_names if col not in ["input_values", "labels"]],
        batched=True,
        batch_size=8
    )
    eval_dataset = dataset["eval"].map(
        lambda x: preprocess_function(x, feature_extractor, augmentations=None), # No augmentation for eval
        remove_columns=[col for col in dataset["eval"].column_names if col not in ["input_values", "labels"]],
        batched=True,
        batch_size=8
    )
    ...
    print("Sample processed training example keys:", train_dataset[0].keys())
    print("Sample processed label:", train_dataset[0].get("labels"))
    unique_labels = set([ex["labels"] for ex in train_dataset])
    print("Unique labels in training set:", len(unique_labels))

    # Check for data issues
    input_vals = train_dataset[0]["input_values"]
    ...
    # 5. Load the Model with better initialization
    ...
    # 7. Define Training Arguments with stable hyperparameters
    training_args = TrainingArguments(
        ...
        save_steps=400,
        logging_steps=20,
        num_train_epochs=10, # Increased epochs, but early stopping will prevent overfitting
    ...
    trainer = Trainer(
        model=model,
        args=training_args,
        train_dataset=train_dataset, # Use the augmented training set
        eval_dataset=eval_dataset,   # Use the non-augmented eval set
        data_collator=data_collator,
        compute_metrics=compute_metrics,
        callbacks=[NaNGradientCallback(), EarlyStoppingCallback(early_stopping_patience=3)],  # Add EarlyStopping
    )

    logging.info("Trainer initialized.")

    # 9. Test a small batch before full training
    ...
    # 11. Final Evaluation and Model Saving
